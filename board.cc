...
pong_board::pong_board(int w, int h)
    ...
    // mutex for each cell in the grid (along with edges)
    this->mut_grid = new std::mutex[(w+2)*(h+2)];
}
...
int pong_ball::move() {
    ...
    if (cnext.ball) {
        ...
        cnext.ball->b_status.notify_all();  // stop blocking the current threads
        ...
        // lock so we can load this ball onto the warp tunnel queue, then unlock
        cnext.warp->mutex_.lock();
        cnext.warp->ball_queue.push(this);
        cnext.warp->mutex_.unlock();
    
        cnext.warp->w_status.notify_all(); // stop blocking the current threads
        return 0;
