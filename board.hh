...
struct pong_board {
    ...
    std::mutex* mut_grid; // pointer to array of mutexes for each cell of the board
...
struct pong_ball {
    ...
    std::condition_variable_any b_status; // allows us to block
...
struct pong_warp {
    ...
    std::condition_variable_any w_status; // allows us to block
    std::queue<pong_ball*> ball_queue; // store the warp tunnel balls in a queue
    std::mutex mutex_; // mutex to assist with synchronization
