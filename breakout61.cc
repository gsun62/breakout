...
void ball_thread(pong_ball* b) {
    ...
    while (true) {
        ...
        // record the initial x and y to prevent errors while unlocking
        x = b->x;
        y = b->y;

        // lock the cells adjacent to and the cell at the ball's initial position (a 3x3 grid)
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                b->board.mut_grid[(y+j)*(b->board.width+2)+(x+i)].lock();
            }
        }
        
        int mval = b->move();

        // unlock the cells we previously locked
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                b->board.mut_grid[(y+j)*(b->board.width+2)+(x+i)].unlock();
            }
        }
        ...
        if (mval > 0) {
            ...
            // lock the ball's current position
            b->board.mut_grid[(b->y+1)*(b->board.width+2)+(b->x+1)].lock();

            // we wait until the ball is no longer stopped
            while (b->stopped) {
                b->b_status.wait(b->board.mut_grid[(b->y+1)*(b->board.width+2)+(b->x+1)]);
            }

            // now we unlock the position of the ball
            b->board.mut_grid[(b->y+1)*(b->board.width+2)+(b->x+1)].unlock();
        }
    }
    delete b;
    --nrunning;
...
void warp_thread(pong_warp* w) {
    ...
    while (true) {
        // lock the warp tunnel
        w->mutex_.lock();

        // wait for a ball to arrive at the tunnel
        while (w->ball_queue.empty()) {
            w->w_status.wait(w->mutex_);
        }

        // we record the current first ball at the tunnel then remove it from the queue
        ...
        // lock the current position of the ball
        w->board.mut_grid[(w->y+1)*(w->board.width+2)+(w->x+1)].lock();
        ...
        b->y = w->y;
        b->stopped = false;
        b->b_status.notify_all(); // stop blocking after warping the ball

        // unlock the previously locked position of the ball
        w->board.mut_grid[(w->y+1)*(w->board.width+2)+(w->x+1)].unlock();
    }
}
